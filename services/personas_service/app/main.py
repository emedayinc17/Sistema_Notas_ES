...
from app.api.personas import estudiantes, docentes

API_PREFIX = "/v1"

app = FastAPI(title="personas-service", version="1.0.0")
...
@app.get("/")
def root():
    return {"name": "personas-service", "docs": "/docs", "env": settings.APP_ENV}

@app.get("/health")
def health():
    with engine_grades.connect() as conn:
        conn.execute(text("SELECT 1"))
    return {"ok": True, "service": "personas", "env": settings.APP_ENV}

app.include_router(estudiantes.router, prefix=API_PREFIX)
