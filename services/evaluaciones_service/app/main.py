...
from app.api.evaluaciones import evaluaciones, notas, reportes

API_PREFIX = "/v1"

app = FastAPI(title="evaluaciones-service", version="1.0.0")
...
@app.get("/")
def root():
    return {"name": "evaluaciones-service", "docs": "/docs", "env": settings.APP_ENV}

@app.get("/health")
def health():
    with engine_grades.connect() as conn:
        conn.execute(text("SELECT 1"))
    return {"ok": True, "service": "evaluaciones", "env": settings.APP_ENV}

app.include_router(evaluaciones.router, prefix=API_PREFIX)
